...
from snail_device import get_snail_parameters,eta,DISPLACEMNT_PULSE_LENGTH,WAITING_TIME_FOR_KERR_DRIFT
import pprint as pp
import numpy as np
import matplotlib.pyplot as plt
import json
from datetime import datetime
dc, g3ac, omega = get_snail_parameters(f_infty=6.95,n=5)
import os
#========================
# Experiment imports
#========================
...
calibration_database_path = "examples/SNAIL/calibration_database_5j.json"
...
if DISPLACE_AND_PLOT:
    ...
    for alpha in alpha_list:
        settings.alpha = alpha
        result = run_displace_and_fit_coherent(snail_osc, settings, data_save_path, calibration_database_path, show_plot=False)
    ...
    for i,eta in enumerate(eta_list):
        ...
        for j,pulse_length in enumerate(pulse_length_list):
            ...
            rho, photon_number_dist, fid, db, angle = run_trisqueeze_and_fit(
                snail_osc, settings, data_save_path, show_plot=False
            )
            ...
            with open(f"{data_save_path}/{snail_osc.name}_trisqueeze_data_{date_time}.json", "w") as f:
                json.dump(data, f)
            ...
            fig.tight_layout()
            plt.savefig(f"{data_save_path}/{snail_osc.name}_trisqueeze_data_{date_time}.png")
    plt.show()
