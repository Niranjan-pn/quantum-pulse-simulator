...
from ats_device import get_ats_parameter_3_junctions, sweet_spot, working_spot_3_junctions, eta, DISPLACEMNT_PULSE_LENGTH,WAITING_TIME_FOR_KERR_DRIFT
import pprint as pp
import numpy as np
import matplotlib.pyplot as plt
import json
from datetime import datetime
dc, ac_plus, ac_minus, omega = get_ats_parameter_3_junctions(*working_spot_3_junctions)
import os
#========================
# Experiment imports
#========================
...
calibration_database_path = "examples/ATS/calibration_database.json"
...
if KERR_DRIFT_ANGLE:
    ...
    for i, alpha in enumerate(probe_alpha):
        ...
        with open(f"{data_save_path}/drift_angles_vs_alpha_{date_time}.json", "w") as f:
            json.dump(data, f)
    plt.tight_layout()
    plt.savefig(f"{data_save_path}/vacuum_population_vs_probe_angle_N3_{date_time}.png")
    plt.show()
    ...
    plt.savefig(f"{data_save_path}/drift_angle_vs_alpha_N3_{date_time}.png")
    plt.show()
...
if DISPLACE_AND_PLOT:
    ...
    with open(f"{data_save_path}/drift_angles_vs_alpha_N3_{date_time}.json", "w") as f:
        json.dump(data, f)
    ...
    plt.savefig(f"{data_save_path}/delta_drift_angle_vs_alpha_N3_{date_time}.png")
    plt.show()
